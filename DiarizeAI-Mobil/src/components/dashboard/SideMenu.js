...
export default function SideMenu({ menuAnim, onClose, onNavigate, onLogout }) {
    ...
    // Arka plan opaklığı (Menü kapalıyken 0, açıkken 0.6)
    ...
    // Menü kapalıyken arka planın tıklanmasını engellemek için ekran dışına atıyoruz
    ...
    return (
        ...
        <Animated.View style={[styles.overlay, { transform: [{ translateX: backdropTranslateX }] }]}>
            ...
            {/* Koyu Arka Plan (Opaklığı parmak hareketine göre değişir) */}
            <Pressable style={styles.backdropContainer} onPress={onClose}>
                <Animated.View style={[styles.backdrop, { opacity: backdropOpacity }]} />
            </Pressable>

            {/* Menü Kutusu (Pozisyonu menuAnim'e bağlı) */}
            <Animated.View 
                style={[
